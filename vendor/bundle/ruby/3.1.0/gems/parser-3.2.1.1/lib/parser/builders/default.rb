      ...
      0.upto(pairs.length - 1) do |i|
        (i + 1).upto(pairs.length - 1) do |j|
          ...
          when :sym, :str, :int, :float
            do_warn = true if key1 == key2
          when :rational, :complex, :regexp
            do_warn = true if @parser.version >= 31 && key1 == key2
          end

          diagnostic :warning, :duplicate_hash_key, nil, key2.loc.expression if do_warn
        end
      end
